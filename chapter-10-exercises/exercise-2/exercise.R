...
salaries$salaries_2018 <- c(salaries$salaries_2017 + salary_adjustments)

# Add a column to the `salaries` data frame that has a value of `TRUE` if the 
# person got a raise (their salary went up)
salaries$raise <- (salaries$salaries_2018 - salaries_2017 > 0)
...
print(salaries$employees[max(salaries$salary_adjustments) == salaries$salary_adjustments])

# What was the largest decrease in salaries between the two years?
print(min(salaries$salary_adjustments))

# What was the name of the employee who recieved largest decrease in salary?
print(salaries$employees[min(salaries$salary_adjustments) == salaries$salary_adjustments])
...
write.csv(salaries, "employee_salaries.csv", row.names = FALSE)
